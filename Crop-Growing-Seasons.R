...
# plot by LandSize (ordered in alphabetical order) 
ggplot(cropsData, aes(x=LandSize)) + 
    geom_bar(fill="#6787b7") + 
    labs(title="Number of Crop Types by Land Size", 
         x = "Land Size", 
...
# plot by LandSize (ordered by size) 
# https://markhneedham.com/blog/2015/02/27/rggplot-controlling-x-axis-order/ 

ggplot(cropsData, aes(x=LandSize)) + 
    geom_bar(fill="#6787b7") + 
    scale_x_discrete(limits = cropsData$LandSize) + 
    labs(title="Number of Crop Types by Land Size", 
         x = "Land Size", 
         y = "Number of crop types") + 
    coord_flip() + 
...
# plot by specific LCTYPE 
# ==> USER INPUT: to plot a different crop, just change the LCTYPE name 

ggplot(data=subset(cropsTimeSeries, LCTYPE=="WHEAT"), aes(x=ind, y=values, colour=LCTYPE )) +
    geom_line() +
    geom_smooth() + 
    theme(legend.position="bottom", legend.direction = "horizontal") + 
    scale_x_discrete(name = "2018", 
