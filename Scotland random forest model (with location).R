...
# This code is for creating a random forest model for Scotland data (2019) WITH a location variable
...
# adding the regional variable to the final nulls removed data ####
# reading in the location file
regional.var <- read.csv("LPIS_NUTS_join.csv")
str(regional.var)
colnames(regional.var)[5] <- "FID_1"

# reading in final nulls removed
final.nulls <- read.csv("Final_NullsRemoved.csv")
str(final.nulls)

# merging zonal data with ground truth data
merged.data <- merge(final.nulls, regional.var, by = "FID_1")

# taking out unnecessary variables
merged.data <- subset(merged.data, select = -c(OBJECTID,Join_Count,TARGET_FID,ID,LOCATION_C,nuts218nm,Shape_Length,Shape_Area))

# assigning merged.data to final nulls data 
final_nulls <- merged.data
table(final_nulls$LCTYPE)


# create a pdf file of plot output
pdf(file="location_randomforest.pdf")
...
# removing "UKM" from the nuts218cd column
final_nulls_unlabelled$nuts218cd <- gsub("UKM", "", final_nulls_unlabelled$nuts218cd)

# converting factor to integer
final_nulls_unlabelled$nuts218cd <- as.numeric(final_nulls_unlabelled$nuts218cd)
...
# removing "UKM" from the nuts218cd column
new_final_nulls$nuts218cd <- gsub("UKM", "", new_final_nulls$nuts218cd)

# converting factor to integer
new_final_nulls$nuts218cd <- as.numeric(new_final_nulls$nuts218cd)
...
# confusion matrix 2 (just a check of the first one)
...
table(train1$nuts218cd)
table(final_nulls_unlabelled$nuts218cd)
...
# Plot actuals and unlabelled precdictions 
grid.arrange(actuals_plot, model_all_predicted_no_labels_plot, nrow = 2, ncol = NULL) 


# OUTPUT FILE ####
# creating an output file of the classified crops 
...
# write.csv(crops_all, file = "crops_all_variables_location.csv")

# predicted classified crops
write.csv(classified_crops_all, file = "all_variables_location.csv")
...
tidy_by_class_accuracies_all <- reshape2::melt(tidy_by_class_accuracies_all)
